      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="p-6 space-y-8">
          ...
          {activeSection === "therapies" && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Therapies</h2>
              {loading ? (
                <p className="text-gray-500">Loading therapies...</p>
              ) : (
                <ul className="space-y-4">
                  {therapies.map((therapy) => (
                    <li key={therapy._id} className="p-4 border rounded-lg hover:shadow transition">
                      <h3 className="font-semibold text-gray-900">{therapy.name}</h3>
                      <p className="text-gray-600 text-sm">{therapy.description}</p>
                      <p className="text-sm text-gray-500">
                        Duration: {therapy.duration} | Price: ₹{therapy.price}
                      </p>
                    </li>
                  ))}
                </ul>
